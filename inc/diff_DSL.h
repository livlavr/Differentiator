...
// #define  DiffDiv(left, right) CreateDiffNode({.type = operation, .data = {.operation      = DIV      }}, Sub(Mul(D(left), CP(right)), Mul(CP(left), D(right))), Pow(CP(right), Num(2)))
// #define  DiffPow(left, right) CreateDiffNode({.type = operation, .data = {.operation      = MUL      }}, CP(right), Pow(CP(left), Sub(CP(right), Num(1)))                             )
